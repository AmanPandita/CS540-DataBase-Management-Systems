...
public:
    ...
    int getSize()
    {
        double size = 8 + 8 + bio.length() + name.length() + 4;
        return size;
    }
    ...
    void writeRecord(fstream &indexFile)
    {
        ...
        int64_t p_id = (int64_t)id;
        int64_t p_mid = (int64_t)manager_id;

        indexFile.write(reinterpret_cast<const char *>(&p_id), sizeof(p_id));
        indexFile << "~" << name << "~" << bio << "~";
        indexFile.write(reinterpret_cast<const char *>(&p_mid), sizeof(p_mid));
        indexFile << "~";
    }
};
...
public:
    ...
    void readRecord(fstream &inputFile)
    {
        ...
        inputFile.read(reinterpret_cast<char *>(&p_id), sizeof(p_id));

        inputFile.ignore(1, '~');
        getline(inputFile, name, '~');
        getline(inputFile, bio, '~');
        inputFile.read(reinterpret_cast<char *>(&p_mid), sizeof(p_mid));

        inputFile.ignore(1, '~');

        int regularId = (int)p_id;
        int regularManagerId = (int)p_mid;
